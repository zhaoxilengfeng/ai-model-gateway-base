...
#   vllm/vllm-openai-cpu:v0.23.0  — render (tokenizer) service，CPU-only 镜像
#                                    vllm launch render 只做 tokenize，不加载模型权重，不需要 GPU
set -e
...
echo "=== 2. vLLM GPU image — model server (from Aliyun) ==="
pull_from_aliyun \
  "vllm-openai:v0.23.0" \
