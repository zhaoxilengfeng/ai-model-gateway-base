...
render pod 使用 `vllm/vllm-openai-cpu:v0.23.0`（CPU-only 镜像），运行 `vllm launch render` 仅做 tokenize，**不需要 GPU，不加载模型权重**。render service 的模型名（args 第一个参数）须与 EPP `token-producer.modelName` 以及 vLLM `--served-model-name` 一致。
