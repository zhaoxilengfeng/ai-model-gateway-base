...
> **模型路径只需包含 tokenizer 文件，不需要模型权重**。模型文件很大时，可单独建一个只含 tokenizer 的轻量目录挂载给 render service，无需拷贝完整权重文件。
> ```bash
> mkdir -p /root/models/glm-4-9b-tokenizer
> cp /home/data/model/glm-4-9b-chat/tokenizer*.json \
>    /home/data/model/glm-4-9b-chat/special_tokens_map.json \
>    /home/data/model/glm-4-9b-chat/*.py \
>    /root/models/glm-4-9b-tokenizer/
> ```
...
```json
{
  "data": [{
    ...
    "root": "/root/models/glm-4-9b-chat"
  }]
}
```
...
```
render  --served-model-name=glm-4-9b \
  --trust-remote-code
          ↕ 必须完全一致
EPP     token-producer.modelName=qwen25-7b-instruct
          ↕ 必须完全一致
vLLM    --served-model-name=glm-4-9b \
  --trust-remote-code
```
